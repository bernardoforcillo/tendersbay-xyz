...
func newFixture(t *testing.T, limiter auth.RateLimiter) *fixture {
	...
	profiles := authtest.NewProfiles(store)
	mail := &authtest.Mailer{}
...
func (f *fixture) signUpVerified(t *testing.T, email string) {
	...
	if err := f.svc.SignUp(ctx, email, authtest.Strong, "Test User", "en-ie", "1.2.3.4"); err != nil {
		t.Fatalf("SignUp: %v", err)
	}
	if err := f.svc.VerifyEmail(ctx, authtest.TokenFrom(t, f.mail.Only(t, authtest.KindVerification).Link), "signup"); err != nil {
...
func TestSignUp_SendsVerificationEmailAndSetsDisplayName(t *testing.T) {
	f := newFixture(t, nil)
	if err := f.svc.SignUp(context.Background(), "a@b.com", authtest.Strong, "Alice", "it-it", "1.2.3.4"); err != nil {
		t.Fatalf("SignUp: %v", err)
	}
	call := f.mail.Only(t, authtest.KindVerification)
...
func TestSignUp_WeakPassword(t *testing.T) {
	...
	if len(f.mail.Sent) != 0 {
		t.Fatalf("weak password must not send mail, got %+v", f.mail.Sent)
	}
}
...
func TestSignUp_DuplicateEmailIsNeutral(t *testing.T) {
	f := newFixture(t, nil)
	ctx := context.Background()
	if err := f.svc.SignUp(ctx, "a@b.com", authtest.Strong, "Alice", "en-ie", "1.2.3.4"); err != nil {
		t.Fatalf("first SignUp: %v", err)
	}
	f.mail.Reset()

	if err := f.svc.SignUp(ctx, "a@b.com", authtest.Strong, "Mallory", "en-ie", "5.6.7.8"); err != nil {
		t.Fatalf("duplicate SignUp returned %v, want nil", err)
	}
	if f.mail.Count(authtest.KindVerification) != 0 {
		t.Fatalf("duplicate signup must not mail a redeemable link")
	}
	exists := f.mail.Only(t, authtest.KindAccountExists)
...
func TestSignUp_NormalizesEmail(t *testing.T) {
	f := newFixture(t, nil)
	if err := f.svc.SignUp(context.Background(), "  A@B.CoM ", authtest.Strong, "Alice", "en-ie", "1.2.3.4"); err != nil {
		t.Fatalf("SignUp: %v", err)
	}
	if _, err := f.profiles.FindByEmail(context.Background(), "a@b.com"); err != nil {
		t.Fatalf("normalized lookup: %v", err)
	}
	if f.mail.Only(t, authtest.KindVerification).To != "a@b.com" {
...
func TestLogin_UnverifiedEmail(t *testing.T) {
	...
	_, err := f.svc.Login(ctx, "a@b.com", authtest.Strong, "1.2.3.4", "test-agent")
...
func TestLogin_UnknownEmailIsInvalidCreds(t *testing.T) {
	f := newFixture(t, nil)
	_, err := f.svc.Login(context.Background(), "nobody@b.com", authtest.Strong, "1.2.3.4", "test-agent")
...
func TestLogin_NormalizesEmail(t *testing.T) {
	f := newFixture(t, nil)
	f.signUpVerified(t, "a@b.com")
	res, err := f.svc.Login(context.Background(), " A@B.com ", authtest.Strong, "1.2.3.4", "test-agent")
	if err != nil {
...
func TestLogin_RateLimitedPerIP(t *testing.T) {
	...
	_, err := f.svc.Login(context.Background(), "a@b.com", authtest.Strong, "1.2.3.4", "test-agent")
...
func TestLogin_LimiterErrorFailsOpen(t *testing.T) {
	...
	f.signUpVerified(t, "a@b.com")
	if _, err := f.svc.Login(context.Background(), "a@b.com", authtest.Strong, "1.2.3.4", "test-agent"); err != nil {
		t.Fatalf("Login: %v", err)
	}
}
...
func TestLoginRefreshLogout_RotatesAndRevokes(t *testing.T) {
	...
	first, err := f.svc.Login(ctx, "a@b.com", authtest.Strong, "1.2.3.4", "test-agent")
	if err != nil {
		t.Fatalf("Login: %v", err)
	}
...
func TestVerifyAccessToken_RejectsTampered(t *testing.T) {
	...
	ctx := context.Background()
	f.signUpVerified(t, "a@b.com")
	res, err := f.svc.Login(ctx, "a@b.com", authtest.Strong, "1.2.3.4", "test-agent")
	if err != nil {
		t.Fatalf("Login: %v", err)
	}
...
func TestForgotPassword_PerEmailCapIsSilent(t *testing.T) {
	...
	if f.mail.Count(authtest.KindReset) != 0 {
...
func TestForgotPassword_UnknownAddressIsSilent(t *testing.T) {
	...
	if len(f.mail.Sent) != 0 {
		t.Fatalf("unknown address must not be mailed, got %+v", f.mail.Sent)
	}
}
...
func TestResetPassword_RotatesCredentialAndRevokesSessions(t *testing.T) {
	...
	live, err := f.svc.Login(ctx, "a@b.com", authtest.Strong, "1.2.3.4", "test-agent")
	if err != nil {
		t.Fatalf("Login: %v", err)
	}
	...
	if err := f.svc.ResetPassword(ctx, authtest.TokenFrom(t, f.mail.Only(t, authtest.KindReset).Link), next); err != nil {
		t.Fatalf("ResetPassword: %v", err)
	}
	if _, err := f.svc.Login(ctx, "a@b.com", authtest.Strong, "1.2.3.4", "test-agent"); !errors.Is(err, auth.ErrInvalidCreds) {
...
func TestResetPassword_WeakPassword(t *testing.T) {
	...
	err := f.svc.ResetPassword(ctx, authtest.TokenFrom(t, f.mail.Only(t, authtest.KindReset).Link), "short")
	if !errors.Is(err, auth.ErrWeakPassword) {
		t.Fatalf("err = %v, want ErrWeakPassword", err)
	}
}
...
func TestVerifyEmail_SpentTokenIsRejected(t *testing.T) {
	...
	tok := authtest.TokenFrom(t, f.mail.Only(t, authtest.KindVerification).Link)
